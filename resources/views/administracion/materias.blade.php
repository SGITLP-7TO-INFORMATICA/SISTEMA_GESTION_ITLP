          ...
          <tr id="fila-{{ $m->id }}"
            ...
            <td class="px-4 py-[10px]">
              <div class="flex items-center justify-end gap-1.5">
                ...
                {{-- Eliminar --}}
                <form id="del-materia-{{ $m->id }}" method="POST" action="{{ route('administracion.materias.eliminar', $m->id) }}">
                  @csrf
                  @method('DELETE')
                  <button type="button" title="Eliminar materia"
                    onclick="if(confirm('¿Eliminar la materia «{{ addslashes($m->Nombre) }}»? Esta acción no se puede deshacer.')) document.getElementById('del-materia-{{ $m->id }}').submit()"
