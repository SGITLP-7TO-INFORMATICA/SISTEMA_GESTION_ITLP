...
<div class="flex items-start gap-4 flex-wrap">
  ...
  <div class="flex flex-col gap-4 flex-1 min-w-[340px] max-w-[800px]">
    ...
    {{-- Alerta de éxito --}}
    @if (session('success'))
      <div class="bg-success/[0.08] border border-success/25 rounded-lg px-4 py-2.5 text-[13px] text-success flex items-center gap-2 fade-1">
    ...
    {{-- Alerta de error --}}
    @if (session('error'))
      <div class="bg-danger/[0.08] border border-danger/25 rounded-lg px-4 py-2.5 text-[13px] text-danger flex items-center gap-2 fade-1">
    ...
    {{-- Banner edición --}}
    <div id="banner-edicion" class="bg-accent/[0.08] border border-accent/25 rounded-lg px-4 py-2.5 text-[12.5px] text-accent2 items-center gap-[10px]">
    ...
    {{-- ── FORMULARIO ── --}}
    <form method="POST" action="{{ route('administracion.cursos.guardar') }}" id="main-form" class="fade-2">
      @csrf
        ...
        <table class="w-full border-collapse">
          <thead>
            <tr>
              ...
              <th class="px-4 py-[9px] text-[10.5px] font-semibold text-muted uppercase tracking-[0.1em] border-b border-dim bg-surface2/60 text-center w-[130px]">Grupo taller</th>
              <th class="px-4 py-[9px] text-[10.5px] font-semibold text-muted uppercase tracking-[0.1em] border-b border-dim bg-surface2/60 text-center w-[80px]">Alumnos</th>
              <th class="px-4 py-[9px] border-b border-dim bg-surface2/60 w-[80px]"></th>
            </tr>
          </thead>
          <tbody>
            @foreach ($cursos as $c)
              <tr id="fila-{{ $c->id }}"
                ...
                <td class="px-4 py-[10px]">
                  <div class="flex items-center justify-end gap-1.5">
                    ...
                    {{-- Eliminar --}}
                    <form id="del-curso-{{ $c->id }}" method="POST" action="{{ route('administracion.cursos.eliminar', $c->id) }}">
                      @csrf
                      @method('DELETE')
                      <button type="button" title="Eliminar curso"
                        onclick="if(confirm('¿Eliminar el curso «{{ addslashes($c->nombre) }}»? Esta acción no se puede deshacer.')) document.getElementById('del-curso-{{ $c->id }}').submit()"
...
<script>
  ...
  function cargarCurso(id, nombre, idAnio, grupoTaller) {
    ...
    setModoEditar(true);
    Livewire.dispatch('curso-seleccionado', { cursoId: id });
  ...
  function cancelarEdicion() {
    ...
    document.querySelectorAll('tbody tr').forEach(r => r.classList.remove('bg-accent/10'));
    setModoEditar(false);
    Livewire.dispatch('curso-cancelado');
  }
