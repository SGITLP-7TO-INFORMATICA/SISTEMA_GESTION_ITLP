...
CREATE TABLE `alumnos_anios` (
  ...
  `Fecha_Apertura` date NOT NULL DEFAULT '2026-04-01',
  PRIMARY KEY (`id`)
...
CREATE TABLE `alumnos_asistencias` (
  ...
  CONSTRAINT `fk_asist_curso` FOREIGN KEY (`id_Curso`) REFERENCES `alumnos_cursos` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
...
CREATE TABLE `alumnos_cursos` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `Nombre` varchar(255) NOT NULL,
  `id_Grupo_Taller` int(10) unsigned DEFAULT NULL,
  `Modalidad` enum('INFORMATICA','ELECTROMECANICA') DEFAULT NULL,
  `Fecha_Creacion` datetime DEFAULT NULL,
  `Fecha_Actualizacion` datetime DEFAULT NULL,
  `id_Anio` int(11) DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `fk_cursos_grupo_taller` (`id_Grupo_Taller`),
  ...
  CONSTRAINT `fk_cursos_grupo_taller` FOREIGN KEY (`id_Grupo_Taller`) REFERENCES `alumnos_grupos_taller` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
...
--
-- Table structure for table `alumnos_grupos_taller`
--

DROP TABLE IF EXISTS `alumnos_grupos_taller`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
...
CREATE TABLE `alumnos_notas_trabajos` (
  ...
  CONSTRAINT `fk_alu_notas_tra_id_trabajo` FOREIGN KEY (`id_trabajo`) REFERENCES `docentes_trabajos` (`id_docente_creador`) ON DELETE NO ACTION ON UPDATE NO ACTION
...
CREATE TABLE `docentes_trabajos` (
  ...
  KEY `fk_doc_tra_materia_dictado_idx` (`id_materia_dictado`),
  ...
  CONSTRAINT `fk_doc_tra_materia_dictado` FOREIGN KEY (`id_materia_dictado`) REFERENCES `materias_dictado` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `materias_dictado` (
  ...
  PRIMARY KEY (`id`),
  KEY `fk_dictado_curso` (`id_Curso`),
  KEY `fk_dictado_materia` (`id_Materia`),
  KEY `fk_dictado_modulo_idx` (`id_Modulo_Horario`),
  CONSTRAINT `fk_dictado_curso` FOREIGN KEY (`id_Curso`) REFERENCES `alumnos_cursos` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
...
--
-- Table structure for table `mxm_alumnos_alumnos_anios`
--

DROP TABLE IF EXISTS `mxm_alumnos_alumnos_anios`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `mxm_alumnos_alumnos_anios` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `id_Alumno` int(10) unsigned NOT NULL,
  `id_Curso` int(10) unsigned NOT NULL,
  `Fecha_Asignacion` date NOT NULL,
  PRIMARY KEY (`id`),
  KEY `fk_mxm_aa_alumno` (`id_Alumno`),
  KEY `fk_mxm_aa_curso` (`id_Curso`),
  CONSTRAINT `fk_mxm_aa_alumno` FOREIGN KEY (`id_Alumno`) REFERENCES `alumnos` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `fk_mxm_aa_curso` FOREIGN KEY (`id_Curso`) REFERENCES `alumnos_cursos` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
...
/*!50001 VIEW `view_docentes_materias_dictadas` AS select `u`.`id` AS `USUARIO_ID`,`md`.`id` AS `DICTADO_ID`,`mxm_dmd`.`id` AS `DOCENTE_ID`,concat(`u`.`nombre`,' ',`u`.`apellido`) AS `DOCENTE_NOMBRE`,`m`.`id` AS `MATERIA_ID`,`m`.`Nombre` AS `MATERIA_NOMBRE`,`c`.`id` AS `CURSO_ID`,`c`.`Nombre` AS `CURSO_NOMBRE` from (((((`materias_dictado` `md` left join `mxm_docente_materia_dictada` `mxm_dmd` on(`mxm_dmd`.`id_Materia_Dictado` = `md`.`id`)) left join `docentes` `d` on(`mxm_dmd`.`id_Docente` = `d`.`id`)) left join `usuarios` `u` on(`d`.`id_usuario` = `u`.`id`)) left join `materias` `m` on(`md`.`id_Materia` = `m`.`id`)) left join `alumnos_cursos` `c` on(`md`.`id_Curso` = `c`.`id`)) */;
...
-- Dump completed on 2026-04-22 23:00:59
